...
def train_one_epoch(
    ...
    for it, (imgs, masks) in enumerate(metric_logger.log_every(train_loader, 10, header)):
        ...
        if it < 3:
            preds = outputs['preds']
            print(f"[DIAG] it={it} decode_loss={logs.get('decode_loss', -1):.4f} detect_loss={logs.get('detect_loss', -1):.4f} "
                  f"preds_msg_mean={preds[:,1:].mean().item():.4f} preds_msg_std={preds[:,1:].std().item():.4f} "
                  f"mask_frac={outputs['masks'].float().mean().item():.3f} selected_aug={outputs['selected_aug']}", flush=True)
...
def eval_full(
    ...
    for it, (imgs, masks) in enumerate(metric_logger.log_every(val_loader, 10, header)):
        ...
        # Combine validation masks and segmentation masks (非 COCO 时 masks 为标量 0，忽略)
        if torch.is_tensor(masks) and masks.numel() > 1 and len(masks.shape) != 1:
            ...
            inverse_resize_mask = transforms.Resize((imgs.shape[-2], imgs.shape[-1]), interpolation=transforms.InterpolationMode.NEAREST)
