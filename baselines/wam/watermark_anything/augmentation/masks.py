...
class FullMaskEmbedder:
    ...
    def sample_representative_masks(self, img, nb=5):
        """Return nb full masks (all-ones) for evaluation. Shape (nb,1,H,W)."""
        mask = np.ones((1, img.shape[-2], img.shape[-1]), np.float32)
        return torch.from_numpy(np.stack([mask] * nb, 0))

    def sample_multiwm_masks(self, img, nb_times=None):
        """Return nb_times full masks (all-ones), for multi-wm eval with full mask."""
        nb = nb_times or 5
        mask = np.ones((1, 1, img.shape[-2], img.shape[-1]), np.float32)
        union = torch.from_numpy(np.concatenate([mask] * nb, 0))
        individuals = torch.from_numpy(np.concatenate([mask] * nb, 0))
        return union, individuals

    def __call__(self, img, iter_i=None, raw_image=None, masks=None, no_overlap=False, nb_times=None):
...
class MixedMaskEmbedder:
    ...
    def __call__(self, 
        ...
        if isinstance(gen, CocoSegmentationMaskEmbedder):
            if masks is None or (isinstance(masks, (int, float)) or (hasattr(masks, 'ndim') and masks.ndim < 3)):
                # 非 COCO 数据（masks 为标量 0）：退化为全 1 掩码
                result = torch.from_numpy(np.ones((1, 1, imgs.shape[-2], imgs.shape[-1]), np.float32))
                if imgs.ndim == 4:
                    result = result.repeat(imgs.shape[0], 1, 1, 1)
            else:
                result = masks
        else:
