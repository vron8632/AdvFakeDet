...
\begin{abstract}
Privacy-cloaking techniques such as Fawkes protect face images from unauthorized face recognition (FR) by adding imperceptible perturbations; however, cloaked images carry no verifiable provenance, allowing anyone to republish a victim's cloaked face without accountability. This paper studies \emph{provenance-verifiable privacy cloaking}: embedding a decodable neural watermark into cloaked faces so that FR evasion is preserved, the watermark remains detectable, and detection is robust to JPEG compression. We first quantify the deployment bottleneck: WAM watermark decoding is highly JPEG-fragile, and cloaked faces are slightly more fragile than clean ones---under JPEG75 recompression (typical sharing platforms), TPR@95\% collapses from 99.4\% (true clean) to 6.0\% on cloaked faces (14.0\% on clean), while at the pixel level cloaking alone does not degrade decoding (99.89\% vs.\ 99.84\% bit accuracy). We then propose two complementary mechanisms. (1) \emph{Safety-region guidance} (SRG) computes an FR-sensitivity map from surrogate gradients and embeds watermarks preferentially in FR-insensitive regions, raising cloaking retention under strong watermarking from 36.1\% to 42.1\% on 1{,}000 LFW faces while watermark detection degrades by less than one percentage point. (2) \emph{Strength-calibrated embedding with BER-based detection} increases watermark detectability under JPEG75 from 6.0\% to 57.9\% (TPR@95\%) and to 100\%/99.6\%/93.7\% under BER$\le$0.25 detection for true-clean/JPEG80/JPEG50, at a false-positive rate of 0.2--0.6\% under deployment conditions (0\% at $\theta\le0.15$). Under stronger cloaking (Fawkes mid, 85\% protection success rate), watermarking coexists losslessly: retention is unchanged and detection reaches 95--100\%. Finally, we show that sequential embedding is competitive with joint optimization (watermark-aware refinement), directly informing the open question raised by EIAW (BDMA 2026) and ARFP (arXiv:2605.01217). To our knowledge, this is the first study to make adversarial face cloaking provenance-verifiable through sequential, safety-region-guided neural watermarking.
\end{abstract}
...
\label{sec:main}
Table~\ref{tab:main} reports the main comparison with $\beta{=}6$, evaluated at true clean and under JPEG80/50. Global strong watermarking (B) breaks cloaking: PSR drops from 47.4\% to 36.1\%. Safety-region guidance (C) recovers PSR to 42.1\% with less than one percentage point of watermark loss (100\% vs.\ 100\% at true clean; 99.0\% vs.\ 99.6\% at JPEG80), confirming that SRG embeds watermark energy away from FR-critical regions.
...
We studied provenance-verifiable privacy cloaking for face images. Our experiments establish four findings: (i) the deployment bottleneck of watermarking cloaked faces is JPEG recompression, to which cloaked faces are measurably more fragile (TPR@95\% 99.4\%$\rightarrow$6.0\% under JPEG75; 85.2\% vs.\ 88.3\% bit accuracy vs.\ clean), while cloaking alone is harmless at the pixel level; (ii) safety-region guidance and strength-calibrated embedding with BER-based detection jointly restore watermark detectability to 93--100\% at 0--0.6\% FPR while preserving cloaking retention; (iii) under strong cloaking, sequential embedding is competitive with joint optimization and outperforms the reverse embedding order; and (iv) a SOTA neural watermarking baseline (InvisMark) fails under JPEG on the same protocol, underscoring the role of BER-based detection. These results provide the first practical evidence that adversarial face cloaking can be made provenance-verifiable without sacrificing privacy protection, and they inform the sequential-vs-joint debate in adversarial watermarking.
